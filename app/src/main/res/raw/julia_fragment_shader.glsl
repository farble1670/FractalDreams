...
// Julia calculation constants
uniform vec2 u_julia_c;  // The constant c parameter that defines this Julia set
...
void main() {
    ...
    vec2 pixel_pos = rotated + u_target;

    // Julia iteration: z starts at pixel position, c is constant
    vec2 z = pixel_pos;
    int iterations = 0;
    ...
    for (int i = 0; i < u_max_iterations; i++) {
        ...
        // z = z^2 + u_julia_c
        float temp = z.x * z.x - z.y * z.y + u_julia_c.x;
